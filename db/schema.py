...
async def applySchema(
    ...
    *,
    ...
    await db.execute("""
        ...
        """)
    await db.execute("""
        CREATE TABLE IF NOT EXISTS training_result_export_state (
            messageId INTEGER PRIMARY KEY,
            hostRobloxUserId INTEGER,
            hostRobloxUsername TEXT NOT NULL DEFAULT '',
            hostResolveStatus TEXT NOT NULL DEFAULT 'PENDING',
            hostResolveError TEXT NOT NULL DEFAULT '',
            exportStatus TEXT NOT NULL DEFAULT 'PENDING',
            preparedAt TEXT,
            exportedAt TEXT,
            exportError TEXT NOT NULL DEFAULT '',
            sheetRowNumber INTEGER NOT NULL DEFAULT 0,
            createdAt TEXT NOT NULL DEFAULT (datetime('now')),
            updatedAt TEXT NOT NULL DEFAULT (datetime('now'))
        );
        """)
    ...
    if currentVersion < 31:
        await db.execute("DROP TABLE IF EXISTS hg_submissions")
        await db.execute("DROP TABLE IF EXISTS hg_attendance_records")
        await db.execute("DROP TABLE IF EXISTS hg_sentry_logs")
        await db.execute("DROP TABLE IF EXISTS hg_event_records")
    await db.execute("""
    ...
    await db.execute("""
        CREATE TABLE IF NOT EXISTS hg_event_records (
            ...
            platoon TEXT NOT NULL DEFAULT 'none',
            status TEXT NOT NULL DEFAULT 'OPEN',
    ...
    indexStatements = (
        ...
        "CREATE INDEX IF NOT EXISTS idx_training_result_export_status ON training_result_export_state(exportStatus, updatedAt)",
        "CREATE INDEX IF NOT EXISTS idx_training_result_export_host_status ON training_result_export_state(hostResolveStatus, updatedAt)",
        ...
        "DROP INDEX IF EXISTS idx_hg_submissions_target",
        "DROP INDEX IF EXISTS idx_hg_attendance_target",
        "DROP INDEX IF EXISTS idx_hg_event_records_clockin_session",
        "CREATE INDEX IF NOT EXISTS idx_hg_submissions_status ON hg_submissions(guildId, status, createdAt)",
        "CREATE INDEX IF NOT EXISTS idx_hg_submissions_target ON hg_submissions(targetUserId, createdAt)",
        ...
        "CREATE INDEX IF NOT EXISTS idx_hg_attendance_event ON hg_attendance_records(eventId, userId)",
