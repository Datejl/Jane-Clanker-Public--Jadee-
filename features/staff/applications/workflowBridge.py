...
async def getApplicationWorkflowHistorySummary(
    ...
    *,
    ...
    for row in reversed(rows):
        ...
        transitionText = toLabel
        if note:
            transitionText = f"{toLabel} - {note}"
        timestampText = _discordTimestamp(row.get("createdAt"))
        lines.append(f"{timestampText}: {transitionText} ({actorText})")
    return "\n".join(lines)[:1024]
